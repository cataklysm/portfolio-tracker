 ...
 * listing's selected provider and that provider's symbol. Used so event feeds are
 * fetched from the provider chosen per instrument, not a hardcoded one. The
 * earnings/corporate-actions/news capabilities share one provider selection, so
 * the events refresh resolves a single plan (capability `earnings`).
 *
