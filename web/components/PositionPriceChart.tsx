...
export function PositionPriceChart({ data, currency, locale, dailyPositive }: Props) {
  const sorted = useMemo(() => [...data].sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime()), [data])
  const latestTime = sorted.length > 0 ? new Date(sorted[sorted.length - 1]!.time).getTime() : 0
  const seriesByPeriod = useMemo(() => new Map(PERIODS.map((period) => [
    period.key,
    period.durationMs === null ? sorted : sorted.filter((point) => new Date(point.time).getTime() >= latestTime - period.durationMs!),
  ])), [latestTime, sorted])
  ...
  return (
    <>
      ...
      <div className="p-4">
        {visible.length >= 2 ? <PriceChart data={visible} currency={currency} positive={periodReturn !== null ? periodReturn >= 0 : dailyPositive} locale={locale} /> : <div className="flex h-44 items-center justify-center text-xs text-[var(--app-text-faint)]">Price history is not available for this period.</div>}
      </div>
    </>
  )
}
