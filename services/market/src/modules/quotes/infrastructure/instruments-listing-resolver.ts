...
/**
 * Resolves listing -> provider symbol/currency from the instruments service's
 * internal resolve endpoint. Used by the refresh scheduler (a background worker
 * with no user token); the endpoint is internal-only and must be network- or
 * gateway-restricted in deployment. Listings without an explicit provider
 * mapping are skipped (omitted from the result) rather than guessed from the
 * bare listing symbol — guessing produces wrong/garbage quotes, so a quote is
 * only fetched once a correct provider symbol has been set.
 */
export class InstrumentsListingResolver implements ListingResolver {
  ...
  async resolve(listingIds: string[], provider: string): Promise<Map<string, ResolvedListing>> {
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.warn({ status: response.status, error_code: 'instruments_resolve_failed' }, 'Listing resolve failed');
        return map;
      }
      const items = (await response.json()) as ResolveResponseItem[];
      for (const item of items) {
        // No explicit provider symbol → skip (don't guess from the bare symbol).
        if (!item.provider_identifier) continue;
