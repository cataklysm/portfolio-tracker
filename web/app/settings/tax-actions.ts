...
/** Saves the user's residence-level tax settings (validated server-side against the rule schema). */
export async function saveUserTaxSettingsAction(
  country: string,
  settings: Record<string, unknown>,
): Promise<Result> {
  let resp: Response
  try {
    resp = await apiFetch("/tax-settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ country, settings }),
    })
  } catch {
    return { error: "Cannot reach the gateway." }
  }
  if (!resp.ok) return { error: await problemDetail(resp, "Failed to save tax settings.") }
